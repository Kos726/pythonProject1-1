...
@dp.message(F.text.lower().in_({"привет", "здрасте", "хелоу"}))  # применяем фильтр по тексту с понижением регистра
async def first_message(message: types.Message):
    print("Приветствие")
    await message.answer(f'Добрый день! Для запуска бота используетй команду /start')  # сообщение в ответ


@dp.message(Command('start'))  # запрограммированная функция команды старта
async def start_message(message: types.Message):
    print("Приветствие")
    await message.answer('Рады Вас видеть в нашем тестовом боте. Бот может реагировать на команду /info, '
                         'отвечать на текст ff и Urban в любом регистре, повторять сообщения пользователя')
